...
export class AuthController {
  ...
  verifyEmail(@Body() dto: VerifyOtpDto) { return this.authService.verifyEmail(dto.userId, dto.code); }

  @Post('login') @HttpCode(200)
  login(@Body() dto: LoginDto) { return this.authService.login(dto); }

  @Post('login/verify-otp') @HttpCode(200)
  ...
  resendOtp(@Body() dto: ResendOtpDto, @Query('purpose') purpose: 'verify-email' | 'login' = 'verify-email') {
  ...
  @Patch('me') @UseGuards(AuthGuard('jwt')) @HttpCode(200)
  ...
  @Post('2fa/totp/setup')    @UseGuards(AuthGuard('jwt'), PassengerGuard) @HttpCode(200)
  setupTotp(@CurrentUser() user: User) { return this.authService.setupTotp(user); }

  @Post('2fa/totp/confirm')  @UseGuards(AuthGuard('jwt'), PassengerGuard) @HttpCode(200)
  confirmTotpSetup(@CurrentUser() user: User, @Body() body: { code: string }) {
    return this.authService.confirmTotpSetup(user.id, body.code);
  }

  @Delete('2fa/totp') @UseGuards(AuthGuard('jwt'), PassengerGuard) @HttpCode(200)
  disableTotp(@CurrentUser() user: User) { return this.authService.disableTotp(user.id); }

  // ─── Email 2FA ────────────────────────────────────────────────────────────

  @Patch('2fa') @UseGuards(AuthGuard('jwt')) @HttpCode(200)
  ...
  @Post('refresh') @UseGuards(AuthGuard('jwt-refresh')) @HttpCode(200)
  refresh(@CurrentUser() user: User) { return this.authService.refresh(user); }

  @Post('logout') @UseGuards(AuthGuard('jwt')) @HttpCode(200)
  logout(@CurrentUser() user: User) { return this.authService.logout(user.id); }
}